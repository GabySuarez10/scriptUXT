...
(function () {
  ...
  const FEEDBACK_SCROLL_IDLE_MS = 700;
  const FEEDBACK_SCROLL_MIN_DELTA = 120;

  const sessionKey = 'uxt_fb_session';
  const countKey = 'uxt_fb_count';
  const questionsKey = 'uxt_fb_questions';
  const pageScope = (window.location.origin + window.location.pathname)
    .replace(/[^a-z0-9]/gi, '_')
    .slice(-90);
  const clicksKey = 'uxt_fb_clicks_' + pageScope;
  const scrollsKey = 'uxt_fb_scrolls_' + pageScope;
  const timeKey = 'uxt_fb_time_start_' + pageScope;
  const lastScrollYKey = 'uxt_fb_last_scroll_y_' + pageScope;
  ...
  let feedbackScrollTimeout = null;
  let lastFeedbackScrollY = parseInt(sessionStorage.getItem(lastScrollYKey) || String(window.scrollY || 0), 10);
  ...
  window.addEventListener('scroll', function () {
    if (!popupActive) {
      clearTimeout(feedbackScrollTimeout);

      feedbackScrollTimeout = setTimeout(function () {
        const currentScrollY = Math.round(window.scrollY || document.documentElement.scrollTop || 0);
        const scrollDelta = Math.abs(currentScrollY - lastFeedbackScrollY);

        if (scrollDelta >= FEEDBACK_SCROLL_MIN_DELTA) {
          scrollsSinceLast++;
          lastFeedbackScrollY = currentScrollY;
          sessionStorage.setItem(scrollsKey, String(scrollsSinceLast));
          sessionStorage.setItem(lastScrollYKey, String(lastFeedbackScrollY));
          checkFeedbackTrigger();
        }
      }, FEEDBACK_SCROLL_IDLE_MS);
    }
  }, { passive: true });
  ...
  function showFeedbackPopup() {
    ...
    scrollsSinceLast = 0;
    timeStart = Date.now();
    lastFeedbackScrollY = Math.round(window.scrollY || document.documentElement.scrollTop || 0);
...
// ══════════════════════════════════════════════════════════════════════
// CAPTURA SILENCIOSA DE PANTALLA (html2canvas → Base de Datos)
// Solo se ejecuta una vez por página por navegador
// ══════════════════════════════════════════════════════════════════════
(function () {
  const API_BASE = API_URL.replace('/rutas/visitas', '');
  const SNAPSHOT_ENDPOINT = API_BASE + '/rutas/screenshot/client';
  const pageKey = 'uxt_snap_' + btoa(window.location.origin + window.location.pathname).slice(0, 40);
  ...
  function captureAndSend() {
    ...
    // Dimensiones REALES de la página (esto es lo que se usa para escalar los clics)
    ...
    console.log('📸 UXTracks: Capturando página (' + realWidth + 'x' + realHeight + ')...');

    window.html2canvas(document.body, {
      useCORS: true,
      allowTaint: true,
      scale: 0.5,           // 50% para buena calidad sin exceder límite del servidor
      logging: false,
      x: 0,
      y: 0,
      scrollX: 0,
      ...
      var snapshot = canvas.toDataURL('image/jpeg', 0.5);

      console.log('📸 UXTracks: Imagen generada (' + canvas.width + 'x' + canvas.height + '), tamaño: ' + Math.round(snapshot.length / 1024) + 'KB');

      sessionStorage.setItem(pageKey, '1');

      // IMPORTANTE: Enviamos realWidth/realHeight (dimensiones reales de la página),
      // NO canvas.width/canvas.height (que son los píxeles de la imagen)
      fetch(SNAPSHOT_ENDPOINT, {
        ...
        console.error('❌ UXTracks: Error enviando foto:', err);
      });
    }).catch(function (e) {
      console.error('❌ UXTracks: Error de html2canvas:', e);
    });
  }
